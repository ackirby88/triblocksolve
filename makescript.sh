...
help() {
    ...
    echo "    --clean     -c                    remove local build directories"
    echo "    --distclean -dc                   remove builds and install directories"
    echo "    --release   -opt                  compile the project in optimized mode"
    echo "    --debug     -deb                  compile the project in debug mode"
    echo " "
    echo "  [COMPILER OPTIONS]:"
    echo "     CC=<arg>   cc=<arg>              set the C compiler"
    echo "    CXX=<arg>  cxx=<arg>              set the C++ compiler"
    echo "     FC=<arg>   fc=<arg>              set the Fortran compiler"
    echo " "
    echo "      C_FLAGS=<arg>    c_flags=<arg>  set the C compiler flags"
    echo "    CXX_FLAGS=<arg>  cxx_flags=<arg>  set the C++ compiler flags"
    echo "     FC_FLAGS=<arg>   fc_flags=<arg>  set the Fortran compiler flags"
    echo " "
    echo "   [OCCA MODES]:"
